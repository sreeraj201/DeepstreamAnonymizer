...
{
    GstCaps *caps =
        gst_pad_get_current_caps(decoder_src_pad);

    if (!caps) {
        caps = gst_pad_query_caps(decoder_src_pad,
                                  nullptr);
    }
    ...
    if (!std::strncmp(name, "video", 5)) {
        ...
        if (gst_caps_features_contains(
            ...
            if (!gst_ghost_pad_set_target(
                    GST_GHOST_PAD(bin_ghost_pad),
                    decoder_src_pad)) {

                std::cerr
                    << "Failed to link decoder src "
                  ...
                  gboolean perf_mode)
{
    ...
    g_signal_connect(G_OBJECT(uri_decode_bin),
                     "pad-added",
